...
static inline struct rq* task_rq_lock_oxc(struct task_struct *p)
	        __acquires(rq->lock)
{
        struct rq *rq;
        ...
        for (;;) {
                ...
                if (likely(rq == task_rq_fair_oxc(p)))
                        return rq;
                raw_spin_unlock(&rq->lock);
        }
}

static inline void task_rq_unlock_oxc(struct rq *rq)
        __releases(rq->lock)
{
        raw_spin_unlock(&rq->lock);
}
...
static void enqueue_task_oxc(struct rq *rq, struct task_struct *p, int flags)
{
	...
	//printk("enqueue_task_oxc: pid=%d, oxc_rq_throttled=%d\n", p->pid, oxc_rq->oxc_throttled);
	update_rq_clock(rq);
